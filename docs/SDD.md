...
**Life-support reserve (post-v1.0):** industrial reactions that consume crew-critical inputs (water, gaseous O₂) leave a floor of 5 days × living crew × consumption rate untouched (LSS_RESERVE_DAYS in the reaction framework). Without it, a propellant plant (electrolysis → LOX) drinks every water delivery to zero between resupply landings and settlers die of thirst on a base that exports water by the tonne. The reserve scales with the living roster, so an uncrewed industrial base still runs its chains at full rate.

**Implemented notes (M-Rover):** rovers are entities with fractional tile positions; traverse drain = km × drainKwhPerKm at speedKmh (halved below 50% condition); one RNG failure roll per completed survey (ideal/realistic table) so roverless worlds consume no randomness. Surveys yield survey_science_points (doubled with an ice core); an icy-PSR core sets iceCharacterized by ground truth. During surveys the battery never drops below a 1.15× return reserve (instruments brown out first); recharge at base is a flat 3 kW abstraction below grid resolution. Class specs are composite constants (rover_scout/prospector/sampler — VIPER is the sourced anchor); tile_size_m=250 maps the 64-grid onto ~16 km of rim terrain.
